...
void SolveDVO::computeJacobianOfNowFrame(STATE* keyFrame, STATE* currentFrame, CALIBRATION_PAR* cali,
                                         int level, Eigen::Matrix3f &cR, Eigen::Vector3f &cT,
                                         JacobianLongMatrix &Jcbian)
{
    /// J_{at a pixel} = [ 2x1 grad of now distance ] x [ 2x3 projection jacobian ] x [ del_p / del_psi ]

    Eigen::MatrixXf& _nowDist = currentFrame->distanceTransformEigen[level];


    /// "Step 1" : Gradient of dist transform image <br/>
    //    Eigen::MatrixXf dGx, dGy; //image gradient
    //    imageGradient(_nowDist, dGx, dGy);
    Eigen::MatrixXf& dGx = currentFrame->distanceTransformGradientX[level];
    Eigen::MatrixXf& dGy = currentFrame->distanceTransformGradientY[level];


    Eigen::MatrixXf& _3d = keyFrame->edge_3d[level];
    // Eigen::MatrixXf _2d = keyFrame->edge_2d[level];

    /// "Step 2" : Transform 3d points to ref frame denoted by cR, cT <br/>
    Eigen::MatrixXf cTRep;
    //printf("_3d.cols()=%d keyFrameID=%d level=%d\n", _3d.cols(), keyFrame->id(), level ) ;
    igl::repmat(cT,1,_3d.cols(),cTRep); // repeat cT col-wise (uses IGL library)
    Eigen::MatrixXf _3d_transformed = cR.transpose() * ( _3d - cTRep );

    /// "Step 3" : Corresponding reprojected pts. Project the transformed 3d points onto the image.
    int numOfPoints = _3d_transformed.cols() ;
    reprojections = Eigen::MatrixXf::Zero(2, numOfPoints) ;
    for( int i = 0 ; i < numOfPoints ; i++ )
    {
        reprojections(0, i) = cali->fx[level] * _3d_transformed(0, i) / _3d_transformed(2, i) + cali->cx[level] ;
        reprojections(1, i) = cali->fy[level] * _3d_transformed(1, i) / _3d_transformed(2, i) + cali->cy[level] ;
    }
...
//    _3d_transformed /= 10.0f;
    ...
    int notJ = 0;
    const int nCols = _nowDist.cols()-3;
    const int nRows = _nowDist.rows()-3;

    for( int i=0 ; i<numOfPoints ; i++ )
    {
        if( reprojections(0,i) < 2 || reprojections(0,i) > nCols ||  reprojections(1,i) < 2 || reprojections(1,i)>nRows) {
            notJ++;
            continue;
        }

//        int xx = reprojections(0,i)+0.5;
//        int yy = reprojections(1,i)+0.5;

//        //epsilon(i) = _nowDist( yy, xx );
//        weights(i) = getWeightOf( epsilon(i) );
//        //weights(i) = getWeightOf( _nowDist( yy, xx ) );
...
//        //printf("yy=%d xx=%d\n", yy, xx) ;
        ...
        // G
        //printf("yy=%d xx=%d\n", yy, xx) ;
        G(0) = getInterpolatedElementEigen(dGx, reprojections(0, i), reprojections(1, i) ) ;
        G(1) = getInterpolatedElementEigen(dGy, reprojections(0, i), reprojections(1, i) ) ;
        //        G(0) = dGx(yy,xx);
        //        G(1) = dGy(yy,xx);

        // A1
        A1(0,0) = cali->fx[level]/Z;
        A1(0,1) = 0.;
        A1(0,2) = -cali->fx[level]*X/(Z*Z);
        A1(1,0) = 0.;
        A1(1,1) = cali->fy[level]/Z;
        A1(1,2) = -cali->fy[level]*Y/(Z*Z);
...
float SolveDVO::getReprojectedEpsilons(STATE* keyFrame, STATE* currentFrame, CALIBRATION_PAR* cali, int level )
{
    Eigen::MatrixXf& _nowDist = currentFrame->distanceTransformEigen[level] ;
    ...
    int notJ=0;
    int w = _nowDist.cols() - 3 ;
    int h = _nowDist.rows() - 3 ;

    //printf("reprojections.cols()=%d\n", reprojections.cols() ) ;
    for( int i=0 ; i<reprojections.cols() ; i++ )
    {
        //printf("i=%d %f %f\n", i, reprojections(0,i), reprojections(1,i) ) ;
        if( reprojections(0,i)<2 || reprojections(0,i)> w ||  reprojections(1,i)<2 || reprojections(1,i) > h ) {
            notJ++;
            //epsilon(i) = 200;
...
//#else
//        epsilon(i) = _nowDist( int(reprojections(1,i)+0.5), int(reprojections(0,i)+0.5) );
//#endif
        epsilon(i) = getInterpolatedElementEigen(_nowDist, reprojections(0, i), reprojections(1, i) ) ;
...
void SolveDVO::visualizeDistanceResidueHeatMap( cv::Mat& tmpIm, Eigen::MatrixXi &reprojectionMask, Eigen::MatrixXf &nowDistMap )
{
...
//    cv::eigen2cv( eim, tmpIm );
    ...
    cv::Mat xim ;
    //= cv::Mat::zeros(eim.rows(), eim.cols(), CV_8UC3 );
    ...
    cv::moveWindow( "residues heatmap", 0, 0) ;
}
...
float SolveDVO::runIterations(STATE* keyFrame, STATE* currentFrame, CALIBRATION_PAR* cali,
...
//    descentDirection /= descentDirection.norm();
    ...
    for( int itr=0 ; itr< maxIterations ; itr++ )
    {
        ...
        JacobianLongMatrix Jcbian = Eigen::MatrixXf::Zero(keyFrame->edge_3d[level].cols(), 6 );
        Eigen::Matrix3f cR_32 = cR.cast<float>();
        Eigen::Vector3f cT_32 = cT.cast<float>();


        double t = (double)cvGetTickCount();
//        for( int l= 0; l < optimizedPyramidLevel; l++ ) {
//            printf("_3d.cols()=%d keyFrameID=%d level=%d\n", keyFrame->edge_3d[l].cols(), keyFrame->id(), l ) ;
//        }
        computeJacobianOfNowFrame(keyFrame, currentFrame, cali, level, cR_32, cT_32, Jcbian );
        if ( level == 0 ){
            //printf("compute Jacobian time: %f\n", ((double)cvGetTickCount() - t) / (cvGetTickFrequency() * 1000) );
        }
        ...
        float ratio_of_visible_pts = getReprojectedEpsilons( keyFrame, currentFrame, cali, level );
//        float currentTotalEpsilon = epsilon.norm(); //epsilon.sum() ;
        float currentTotalEpsilon = aggregateEpsilons();
...
//        cR = cR * xRot;
    ...
    return bestTotalEpsilon ;
}


float SolveDVO::optimizeGaussianNewton(STATE* keyFrame, STATE* currentFrame, CALIBRATION_PAR* cali,
                             int level, int maxIterations, Eigen::Matrix3d& cR, Eigen::Vector3d& cT,
                             Eigen::VectorXf& energyAtEachIteration, Eigen::VectorXf& finalEpsilons,
                             Eigen::MatrixXf& finalReprojections, int& bestEnergyIndex, float& finalVisibleRatio)
{
    ...
    for( int itr=0 ; itr< maxIterations ; itr++ )
    {
        ...
        Eigen::MatrixXf& dGy = currentFrame->distanceTransformGradientY[level];
        Eigen::MatrixXf& _3d = keyFrame->edge_3d[level];
        //Eigen::Vector3f* pData = keyFrame->posData[i];
        // Eigen::MatrixXf _2d = keyFrame->edge_2d[level];

        /// "Step 2" : Transform 3d points to ref frame denoted by cR, cT <br/>
        Eigen::MatrixXf cTRep;
        //printf("_3d.cols()=%d keyFrameID=%d level=%d\n", _3d.cols(), keyFrame->id(), level ) ;
        Eigen::Vector3f repmatT = cT.cast<float>() ;
        igl::repmat(repmatT, 1,_3d.cols(),cTRep); // repeat cT col-wise (uses IGL library)
        Eigen::MatrixXf _3d_transformed = (cR.cast<float>()).transpose() * ( _3d - cTRep );
        Eigen::RowVector2f G;
        Eigen::Matrix<float,2,3> A1;
        Eigen::Matrix<float,1,3> oneByThree;
        Eigen::Matrix<float,3,6> A2;
        A2.setZero() ;
        Eigen::Matrix<float,1,6> J_i;
        Eigen::Matrix<double,6,6> JTJ;
        JTJ.setZero() ;
        Eigen::VectorXd JTb(6) ;
        JTb.setZero() ;
        Eigen::Matrix3f wx;
        Eigen::Vector3f tmp;
        int notJ = 0;
        ...
        int numOfPoints = _3d_transformed.cols() ;
        reprojections = Eigen::MatrixXf::Zero(2, numOfPoints) ;
        epsilon = Eigen::VectorXf::Zero( numOfPoints );
        weights = Eigen::VectorXf::Zero( numOfPoints );
        int idx = 0 ;
        A2.block<3,3>(0,0) = -cR.transpose().cast<float>();
//        float sum=0;
...
//        }
//        return sqrt(sum)/epsilon.rows();
        double sumOfError = 0 ;
        for( int i = 0 ; i < numOfPoints ; i++ )
        {
            reprojections(0, i) = cali->fx[level] * _3d_transformed(0, i) / _3d_transformed(2, i) + cali->cx[level] ;
            reprojections(1, i) = cali->fy[level] * _3d_transformed(1, i) / _3d_transformed(2, i) + cali->cy[level] ;
            if( reprojections(0, i) < 2 || reprojections(0, i) > nCols ||  reprojections(1,i) < 0 || reprojections(1,i) > nRows) {
                notJ++;
                continue;
            }
#ifdef MY_SSE
            buf_warped_residual[idx] = getInterpolatedElementEigen(_nowDist, reprojections(0, i), reprojections(1, i) ) ;
            buf_weight_p[idx] = getWeightOf( epsilon(i) );
            buf_warped_x[idx] = _3d_transformed(0,i);
            buf_warped_y[idx] = _3d_transformed(1,i);
            buf_warped_z[idx] = _3d_transformed(2,i);
            buf_warped_dx[idx] = getInterpolatedElementEigen(dGx, reprojections(0, i), reprojections(1, i) ) ;
            buf_warped_dy[idx] = getInterpolatedElementEigen(dGy, reprojections(0, i), reprojections(1, i) ) ;
            idx++ ;
#else
            epsilon(i) = getInterpolatedElementEigen(_nowDist, reprojections(0, i), reprojections(1, i) ) ;            
            weights(i) = getWeightOf( epsilon(i) );
            sumOfError += weights(i)*epsilon(i)*epsilon(i) ;
            //weights(i) = getWeightOf( _nowDist( yy, xx ) );
            float X = _3d_transformed(0,i);
            float Y = _3d_transformed(1,i);
            float Z = _3d_transformed(2,i);
            ...
            // A1
            oneByThree(0, 0) = G(0)*cali->fx[level]/Z;
            oneByThree(0, 1) = G(1)*cali->fy[level]/Z;
            oneByThree(0, 2) = -G(0)*cali->fx[level]*X/(Z*Z) - G(1)*cali->fy[level]*Y/(Z*Z);
//            A1(0,0) = cali->fx[level]/Z;
//            A1(0,1) = 0.;
//            A1(0,2) = -cali->fx[level]*X/(Z*Z);
//            A1(1,0) = 0.;
...
//            A1(1,2) = -cali->fy[level]*Y/(Z*Z);
            ...
            //tmp = cR.transpose().cast<float>() * _3d_transformed.col(i);
            //tmp = cR.transpose().cast<float>() * _3d.col(i);

            A2(0, 4) = -_3d_transformed(2, i);
            A2(0, 5) = _3d_transformed(1, i);
            A2(1, 3) = _3d_transformed(2, i);
            A2(1, 5) = -_3d_transformed(0, i);
            A2(2, 3) = -_3d_transformed(1, i);
            A2(2, 4) = _3d_transformed(0, i);
            //tmp = _3d_transformed.col(i);
            //to_se_3( tmp, wx );
            //A2.block<3,3>(0,3) = wx;
            //J_i = G * A1 * A2;
            ...
            JTJ = JTJ + (weights(i)*J_i.transpose() * J_i).cast<double>() ;
            JTb = JTb - (weights(i)*epsilon(i)*J_i.transpose().cast<double>() ) ;
#endif
        }
        ...
        const __m128 fx = _mm_set1_ps((float)cali->fx[level]);
        const __m128 fy = _mm_set1_ps((float)cali->fy[level]);
        Eigen::Matrix3f cRT = (cR.cast<float>()).transpose() ;
        const __m128 r11 = _mm_set1_ps((float)cRT(0, 0));
        const __m128 r12 = _mm_set1_ps((float)cRT(0, 1));
        const __m128 r13 = _mm_set1_ps((float)cRT(0, 2));
        const __m128 r21 = _mm_set1_ps((float)cRT(1, 0));
        const __m128 r22 = _mm_set1_ps((float)cRT(1, 1));
        const __m128 r23 = _mm_set1_ps((float)cRT(1, 2));
        const __m128 r31 = _mm_set1_ps((float)cRT(2, 0));
        const __m128 r32 = _mm_set1_ps((float)cRT(2, 1));
        const __m128 r33 = _mm_set1_ps((float)cRT(2, 2));
        for( int i = 0 ; i < buf_warped_size-3 ; i += 4 )
        {
            ...
            pz = _mm_rcp_ps(pz);
            __m128 pz2 = _mm_mul_ps(pz, pz) ;

            val3 = _mm_mul_ps(gx, fx) ;
            val1 = _mm_mul_ps(val3, pz) ;
            J31 = val1;

            val4 = _mm_mul_ps(gy, fy) ;
            val2 = _mm_mul_ps(val4, pz) ;
            J32 = val2;

            val3 = _mm_mul_ps(val3, px) ;
            val3 = _mm_mul_ps(val3, pz2) ;
            val3 = _mm_sub_ps(_mm_setr_ps(0,0,0,0),val3);
            val4 = _mm_mul_ps(val4, py) ;
            val4 = _mm_mul_ps(val4, pz2) ;
            val4 = _mm_sub_ps(_mm_setr_ps(0,0,0,0),val4);
            J33 = _mm_add_ps(val3, val4);

            val1 = _mm_mul_ps(J31, r11 ) ;
            val2 = _mm_mul_ps(J32, r21 ) ;
            val3 = _mm_mul_ps(J33, r31 ) ;
            J61 = _mm_sub_ps(_mm_setr_ps(0,0,0,0), val1);
            J61 = _mm_sub_ps(J61, val2);
            J61 = _mm_sub_ps(J61, val3);

            val1 = _mm_mul_ps(J31, r12 ) ;
            val2 = _mm_mul_ps(J32, r22 ) ;
            val3 = _mm_mul_ps(J33, r32 ) ;
            J62 = _mm_sub_ps(_mm_setr_ps(0,0,0,0), val1);
            J62 = _mm_sub_ps(J62, val2);
            J62 = _mm_sub_ps(J62, val3);

            val1 = _mm_mul_ps(J31, r13 ) ;
            val2 = _mm_mul_ps(J32, r23 ) ;
            val3 = _mm_mul_ps(J33, r33 ) ;
            J63 = _mm_sub_ps(_mm_setr_ps(0,0,0,0), val1);
            J63 = _mm_sub_ps(J63, val2);
            J63 = _mm_sub_ps(J63, val3);

            val2 = _mm_mul_ps(J32, pz_o ) ;
            val3 = _mm_mul_ps(J33, py ) ;
            J64 = _mm_sub_ps(val2, val3) ;

            val1 = _mm_mul_ps(J31, pz_o ) ;
            val3 = _mm_mul_ps(J33, px ) ;
            J65 = _mm_sub_ps(val3, val1) ;

            val1 = _mm_mul_ps(J31, py ) ;
            val2 = _mm_mul_ps(J32, px ) ;
            J66 = _mm_sub_ps(val1, val2) ;
        ...
        float ratio_of_visible_pts = (float)(reprojections.cols()-notJ) / (float)reprojections.cols();
        float currentTotalEpsilon = sqrt(sumOfError)/(reprojections.cols()-notJ);
        energyAtEachIteration[itr] = currentTotalEpsilon;
...
#ifdef MY_SSE
        Eigen::VectorXd psi = ls.A.cast<double>().ldlt().solve(ls.b.cast<double>()) ;
#else
        Eigen::VectorXd psi = JTJ.ldlt().solve(JTb) ;
#endif
//        std::cout << "psi = " << psi.transpose() << std::endl;
//        std::cout << "psi2 = " << psi2.transpose() << std::endl;
        ...
        Sophus::SE3d mat = Sophus::SE3d::exp(psi);
        Eigen::Matrix3d xRot = mat.rotationMatrix();
        Eigen::Vector3d xTrans = mat.translation();
        cT += xTrans;
        cR *= xRot;

//        Sophus::SE3d RTse3 ;
//        RTse3.setRotationMatrix( cR );
//        RTse3.translation() = cT ;
//        RTse3 = RTse3 * Sophus::SE3d::exp(psi);
//        cR = RTse3.rotationMatrix();
//        cT = RTse3.translation();
    }
    // Set : f^* = min{ f_best^(k-1), f^k }
    cR = bestcR;
    cT = bestcT;
...
inline float SolveDVO::getWeightOf( float r)
{
    //return 1.0f;
    //return 1.0f + r*r;
...
float SolveDVO::aggregateEpsilons()
{
    //return epsilon.norm();
    float sum=0;
    for( int p=0 ; p<epsilon.rows() ; p++ )
